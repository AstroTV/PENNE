...
void write_observer_ecu_data() {
  ...
  if (ecu_data.observer_code != OK &&
      (ecu_data.observer_code != ecu_data_old.observer_code || ecu_data.observer_id != ecu_data_old.observer_id)) {
    last_observer_warning = micros();
    printf("Setting last_observer_warning\n");
  }
...
void write_ecu_data_to_serial() {
  ...
  switch (ecu_number) {
    case POWERTRAIN:
      ...
      if (ecu_data_old.parking_brake_status != ecu_data.parking_brake_status || send_all_ecu_data_to_gui == true) {
    ...
    case CHASSIS:
      ...
      if (ecu_data_old.turn_signal_indicator != ecu_data.turn_signal_indicator || send_all_ecu_data_to_gui == true) {
